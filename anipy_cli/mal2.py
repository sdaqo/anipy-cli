...
from anipy_cli.provider import FilterCapabilities, Filters, MediaType, Season
...
class MALProviderMapping(DataClassJsonMixin):
    mal_anime: MALAnime
    provider_anime: List[ProviderMapping]
...
class MyAnimeList:
    ...
    def get_search(self, query: str, limit: int = 10, pages: int = 1) -> List[MALAnime]:
    ...
    def make_provider_mapping(
        ...
        for p in providers:
            ...
            if p.FILTER_CAPS & FilterCapabilities.YEAR:
                provider_filters.year = [mal_anime.start_season.year]

            if p.FILTER_CAPS & FilterCapabilities.SEASON:
                provider_filters.season = [
                    Season[mal_anime.start_season.season.upper()]
                ]

            if p.FILTER_CAPS & FilterCapabilities.MEDIA_TYPE:
            ...
            if use_alternative_names:
                for title in titles:
                    results |= set(p.get_search(title))
                    if use_filters:
                        results |= set(p.get_search(title, provider_filters))
            else:
                results |= set(p.get_search(mal_anime.title))
                if use_filters:
                    results |= set(p.get_search(mal_anime.title, provider_filters))

            found[p.NAME] = results
