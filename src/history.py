...
def write_history(link, is_history, is_on_web=False):
    ...
    if in_data == True:
        ...
        if is_history == True:
            ...
            changed_secs = changed_line[1].replace(
                changed_line[1], str(seconds + int(changed_line[1])))
...
def pick_history():
    ...
    counter = 1
    for i in links:
        print(colors.GREEN + "[" + str(counter) + "]" + colors.END + " " +
              str(i.replace(config.gogoanime_url, "")))
        counter += 1
