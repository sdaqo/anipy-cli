...
class DownloadComponent:
    ...
    def download_anime(
        ...
        /,
        after_success_ep: Callable[[Anime, Episode, LanguageTypeEnum], Any] = lambda _,__,___: None,
    ...
    def download_episodes(
        ...
        for ep in eps:
            try:
                self.download_ep(s, downloader, anime, lang, ep)
    ...
    def serve_download_errors(
        ...
        text = ', '.join([f"Episode {i[1]} of {i[0].name}" for i in failures])
        extra_info = " (and the remaining episodes in that series)" if not only_skip_ep_on_err else ''
