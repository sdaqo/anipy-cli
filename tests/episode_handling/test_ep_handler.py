...
def test_gen_link_regular():
    ...
    assert urlparse(show_entry.ep_url).path == '//hyouka-episode-1', "Wrong link generated"
...
def test_gen_link_with_unknown_chars():
    ...
    assert urlparse(show_entry.ep_url).path == '//saenai-heroine-no-sodatekata-2-1', "Wrong link generated"
...
def test_gen_link_special():
    ...
    show_entry = epHandler(
        entry(
            category_url="https://gogoanime.tel/category/86-2nd-season",
            ep=7.5
        )
    ).gen_eplink()

    assert urlparse(show_entry.ep_url).path == '//86-2nd-season-episode-7-5', "Wrong link generated"
